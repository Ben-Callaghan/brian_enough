...
function clickOutside(noClickClass) {
  console.log('called clickOutside');
  $(document).mouseup(function(e)
  {
    var container = $(noClickClass);
    // if the target of the click isn't the container nor a descendant of the container
    if (!container.is(e.target) && container.has(e.target).length === 0)
    {
        $(noClickClass+'Button').removeClass('buttonDown');
        container.hide();
    }
  });
}
...
//buttonDown class toggler for
function xToggle (buttonClass, menuDiv, toggleClass) {
  $(buttonClass).click(function(){
    console.log('called xToggle');
    $(menuDiv).toggleClass(toggleClass);
  });
}
...
tagArray.forEach(function(item){
  $('.tagMenu').append("<div class='tagMenuTag "+item+"Button'>+ " +item+ "</div>");
...
// menu toggle buttons
toggleVisibleButton('.tagMenuButton', '.tagMenu');
toggleVisibleButton('.infoMenuButton', '.infoMenu');
toggleVisibleButton('.tagMenuX', '.tagMenu');
toggleVisibleButton('.infoMenuX', '.infoMenu');

xToggle('.tagMenuX','.tagMenuButton', 'buttonDown');
xToggle('.infoMenuX','.infoMenuButton', 'buttonDown');

//tagMenu buttons
toggleClassButton('.button', 'buttonDown');
...
clickOutside('.infoMenu');
clickOutside('.tagMenu');
